...
let cachedConfig: AdoConfig | null = null;
...
export function loadConfig(): AdoConfig {
  ...
  try {
    ...
    if (!file.organization || !file.pat) {
      throw new Error(
        `Missing required fields in ${configPath}: organization, pat`
      );
    }
    ...
    if (org || pat) {
      throw new Error(
        "Incomplete env config: both AZURE_DEVOPS_ORG and AZURE_DEVOPS_PAT are required"
      );
    }
    throw new Error(
      `Azure DevOps configuration not found. Set AZURE_DEVOPS_ORG + AZURE_DEVOPS_PAT env vars, ` +
        `or create ${configPath}. Error: ${err instanceof Error ? err.message : err}`
    );
  }
}

export function resolveOrg(organizationId?: string): string {
  const config = loadConfig();
  return organizationId || config.organization;
}

export function resolveProject(projectId?: string): string | undefined {
  const config = loadConfig();
...
export function requireProject(projectId?: string): string {
  ...
  if (!resolved) {
    throw new Error(
      "Project ID is required. Either pass projectId or set AZURE_DEVOPS_DEFAULT_PROJECT."
    );
  }
  return resolved;
}
