...
server.tool(
  "authenticate",
  "Authenticate the d-user with Microsoft Graph via device code flow. Required before sending or reading messages. Only needed once per server session.",
  {},
  async () => {
    try {
...
server.tool(
  "send_notification",
  "Send a notification from the d-user to the target user via Teams chat or email.",
  {
    ...
    try {
      ...
      if (config.rateLimit?.enabled) {
        ...
        const cooldownSeconds =
          config.rateLimit.cooldownSeconds[cooldownKey] ??
      ...
      let primaryChannel: Channel =
        channel === "auto" ? (config.channels?.primary || "teams") : channel as Channel;
      const fallbackChannel: Channel = config.channels?.fallback || "email";

      // Send
      let result = await trySendOnChannel(accessToken, config, primaryChannel, message, type as NotificationType | undefined, subject);

      if (!result.success && primaryChannel !== fallbackChannel) {
        result = await trySendOnChannel(accessToken, config, fallbackChannel, message, type as NotificationType | undefined, subject);
        if (result.success) {
...
async function trySendOnChannel(
  ...
  subject?: string
): Promise<{ success: boolean; channel?: string; fallback?: boolean; error?: string }> {
  if (channel === "teams") {
    ...
    const res = await sendEmail(
      accessToken,
      config.targetUser.upn,
      subject || "WOF Notification",
      message,
      config.dUser.upn
    );
...
server.tool(
  "read_messages",
  "Read new messages from the target user in the Teams chat. Returns messages sent since the last read (or last hour on first call).",
  {
    since: z
      .string()
      .optional()
...
server.tool(
  "get_status",
  "Get the current status of the notification server: authentication state, configuration, and rate limit state.",
  {},
  async () => {
    try {
      const config = loadConfig();
...
async function main() {
  ...
  try {
    ...
    process.stderr.write(
      `[wof-notifications] Config loaded: ${config.dUser.upn} -> ${config.targetUser.upn}\n`
    );
  } catch (err) {
    process.stderr.write(
      `[wof-notifications] WARNING: Config error: ${err instanceof Error ? err.message : err}\n` +
