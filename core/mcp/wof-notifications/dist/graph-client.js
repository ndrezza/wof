...
export async function sendTeamsMessage(accessToken, chatId, message, type) {
...
export async function sendEmail(accessToken, recipientUpn, subject, message, senderUpn) {
...
export async function readMessages(accessToken, chatId, targetUserId, since, maxMessages = 10) {
    ...
    for (const msg of data.value) {
        if (msg.messageType !== "message")
            continue;
        if (!msg.from?.user || msg.from.user.id !== targetUserId)
            continue;
